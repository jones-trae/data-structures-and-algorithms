...
const sortByChildren = (charArray) => {
  const sortByChildren = charArray => charArray.sort((f, m) => f.children.length - m.children.length);
  // Solution code here...
};
...
const containsW = (str) => {
  let regex = /w/g;
  return regex.test(str); // Solution code here...
};
...
const isNum = (input) => {
  let regex = /\d/g;
  return regex.test(input);// Solution code here...
};
