...
const addValues = (arr) => {
  let total = arr.reduce((currentT, currentV) => {
    return currentV + currentT;
  }, 0);
...
const addPurchases = (arr) => {
  let totalPurchase = arr.reduce((currentT, currentP) => {
    return currentT + currentP.purchasePrice;
  }, 0);
...
const countNumberOfElements = (arr) => {
  let numberOfElements = arr.reduce((length) => {
    return ++length; // could also use index and return index + 1;
  }, 0);
  return numberOfElements;// Solution code here...
};
...
const returnNames = (arr) => {
  let newArr = arr.reduce ((newArr, para) => {
    newArr.push(para.name);
    return newArr;
  }, []);
...
const reversedString = (str) => {
  let bwStr = str.split('').reduce((prevL, curL) => {
    return curL + prevL;
  }, '');
  return bwStr;// Solution code here...
};
